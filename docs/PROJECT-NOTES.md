...
- `public/film/services/{wide,tall}/NNN.webp` + `src/content/serviceFilm.ts`
  ...
  **generated** by `scripts/build-service-film.py`: `--stand-in` from the
  four service photos (what's there now), or `--master <video> --cuts
  <json>` from the real film (the plan doc has the cuts format). Needs
  `pip install imageio-ffmpeg` for the master (no system ffmpeg here).
- `public/logos/naxis-wordmark-light.png` — the header's logo: the
  ...
  repaints the brown lettering and unmixes its edges against the N's
  gold. The footer, OG images and schema still use the original.
...
- **What We Do** (`ServicesFilm.tsx`, V2): the four services as one film
  ...
  scroll to the end of the pin: in a hold it stays; mid-morph it goes on
  the way it was moving, or to the nearer end. Each service's text
  leaves as its morph begins and arrives as it lands (all four stay in
  the DOM; focusing a hidden one scrolls to it); a rail (right on md+,
  along the foot on phones) shows progress and jumps. Frames load from
  2 screens above, nearest the playhead first; before one is ready the
  poster (frame 0) shows, and a missing frame falls back to the nearest
  loaded one. At rest the picture drifts (scale 1.035 over 10s). The
  stage wrapper is named `hero-service-{slug}` for the service on screen,
  so Learn more morphs into that page's hero. Reduced motion: the old
  `ServicesStack` cards.
...
- Hero C on phones: 42k points instead of 120k, each drawn larger so the
  weave still reads.
- The service film's canvas is never bigger than the frames once they
  cover the stage (~1.14x on a 390px phone, ~0.9x on a 1440 desktop):
  every drawImage is paid per pixel, and more is only upscaling. On the
